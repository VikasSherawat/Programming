...
public class _25 {
	...
	PrintWriter out;
	BufferedReader br;
	double theta = (1 + Math.sqrt(5)) / 2;
	double logTheta = Math.log(theta);
	double l1 = Math.log(Math.sqrt(5)) / logTheta;
	double l2 = Math.log(10) / logTheta;

	public int indexOfFib(int n){
			return (int)Math.ceil(l1 + (n - 1) * l2);
	}

	private void solve(int TURN) throws Exception {
		int n = getInt();
		System.out.println(n==1?1:indexOfFib(n));
	}
